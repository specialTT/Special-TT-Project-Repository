execute store result score temp amplifier run data get entity @s active_effects[{id:"minecraft:absorption"}].hidden_effect.amplifier
execute if score temp amplifier matches ..-1 run scoreboard players add temp amplifier 256
execute store result score temp duration run data get entity @s active_effects[{id:"minecraft:absorption"}].hidden_effect.duration
