...
execute if score result cycle_6s matches 0 run scoreboard players set @e[type=#universal_darkness:using_visual_perception,type=!player] cycle_6s 0
execute if score result cycle_6s matches 0 as @e[type=#universal_darkness:using_visual_perception,type=!player] store result score @s cycle_6s run data get entity @s active_effects[{id:"minecraft:darkness"}].duration
